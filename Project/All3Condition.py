...
def driver():
    n = 100
    cs = np.arange(10,100)
    timeCG = np.zeros(len(cs))
    timeSD = np.zeros(len(cs))
    timeLU = np.zeros(len(cs))
    ...
    timeLU = timeLU
    timeSD = timeSD
    plt.plot(cs,timeCG, label = "Conjugate Gradient")
    plt.plot(cs,timeLU, label = "Gaussian Elimination")
    plt.plot(cs,timeSD, label = "Steepest Descent")
    plt.legend()
